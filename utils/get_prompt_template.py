...
def modify_sot_template(template):
    ...
    try:
        ...
        logger.info(f"Template information after modification function:\n{final_template_str}")
        return template_obj
    ...
    def get_prompt_template(self, method: str, dataset_type: str, betti_number: int, solution_number: int):
        ...
        if method == 'sot':
            ...
            if template_path:
                ...
                modified_template = modify_sot_template(template)
                if modified_template:
                    # 修改这里以正确获取模板字符串
                    template_str = modified_template.messages[0].prompt.template
                    logger.info(f"Template information returned from the template factory:\n{template_str}")
